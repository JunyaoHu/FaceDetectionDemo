...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        vid_capture = cv2.VideoCapture("dynamic.mp4")

        # size = (1920, 1080)
        size = (1280, 720)

        output = cv2.VideoWriter('output/output_video.mp4', cv2.VideoWriter_fourcc(*'XVID'), 30, size)
        ...
        while (vid_capture.isOpened()):
            ...
            if ret:
                ...
                cv2.putText(frame, "fps: "+str(fps)[:6], (50, 50), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1.5,
                            color=(250, 225, 100), thickness=5)
                ...
                for box in boxes:
                    ...
                    cv2.rectangle(frame, (max(0, x1), max(0, y1)), (min(x2, w), min(y2, h)), (0, 0, 255), thickness=3, lineType=cv2.LINE_AA)
